# Balíček: doplnění pool případu `pool_a1_vyveseni` (Závadný plakát)

Tento soubor slouží k **jednorázovému zadání** pro AI: obsahuje **celý prompt** ve stejném znění jako `PROMPT-doplneni-stavajicich-pool-pripadu.md` a níže **vyplněné zadání týmu** pro případ *Závadný plakát*.

**K odeslání:** zkopíruj od sekce „Prompt: doplnění…“ až po konec včetně poznámky pod čarou, a **přilož** soubor `docs/Pripady/pool_a1_vyveseni-Zavadny-plakat.json` (nebo vlož jeho obsah pod zadání).

**Cíl úprav (důležité):** starší položky v `pool_cases_akt1` **dotahujeme na stejnou úroveň** jako u vzoru *Noční směna* / *Ředění léků* — tedy nejen texty u verdiktů, ale i **dostupné herní mechaniky** ve smyslu `.cursor/rules/cases.mdc` (Two-Click, `narrative_lines`, konzistentní stopy, kde dává spisový rámec smysl). Ve výchozím JSONu *Závadného plakátu* ještě **`clue_system` nemusí být** — to není důvod sekci A **vynechat**; cíl je spis **doplnit** tak, aby byl v souladu s novými pravidly a vzorem, **beze změny záměru děje** a bez libovolného maření `effects` (to drží tým).

**Vzor ke struktuře:** `docs/Pripady/pool_a1_tiskarna-vzor_Nocni-smena.json` a případ `pool_a1_lekarna` po modernizaci.
...
## Zadání týmu (doplň / zaškrtni) — **VYPLNĚNO pro `pool_a1_vyveseni`**

- **ID případu (nezamění):** `pool_a1_vyveseni`  
- Přiložený **zdrojový JSON** (celý objekt): *`docs/Pripady/pool_a1_vyveseni-Zavadny-plakat.json` (Sp. zn. 63/1931, titulek *Závadný plakát*) — vlož celý obsah souboru nebo soubor přilož.*  
- Doplň: `[x]` sekce A — `clue_system` + `narrative_lines` u `on_confirm` (pokud ve spisu chybí, **navrhni a doplníš** dle děje a vzoru tiskárny/lekárny; stopy v textu, unikátní `data-clue-id`) / `[x]` lidské dopady u verdictů (B, vč. B2 a inkoust dle vhodnosti) / `[ ]` review_card / `[ ]` redakce textu bez změny děje / `[x]` jiné: kontrola D — `knihovna` jen pokud přidáváš odkazy; unikátnost clue-id  
- Story Bible (`InDubio_StoryBible_v2_Cursor.txt`) / world úryvek (pokud je třeba): *neposílám, pokud nedoplňuješ reálie; při názvech míst/institucí přilož `docs/world-reference.md`.*  
- **Kontext dne (bod 4 u Vstupu):** v `data/days.json` je spis v **dni 1** spolu s lekárnou, tiskárnou, trafikou; v `Mapa_20dni.csv` řádek **Den 1**, sloupec *Patrani_navrh* = prázdné / **std** — drž úřední, napjatý tón hranice národnostní politiky, bez měnění čísel v `effects`.
...
| Soubor | K čemu |
|--------|--------|
| **`docs/Pripady/pool_a1_vyveseni-Zavadny-plakat.json`** | Vstupní JSON (příloha k promptu). |
...
| **`docs/Pripady/pool_a1_tiskarna-vzor_Nocni-smena.json`**, **`docs/Pripady/pool_a1_lekarna-Redeni-leku.json`** | Vzor `clue_system` + `narrative_lines` (tiskárna, lekárna po úpravách). |
| **`docs/scenar/Mapa_20dni.csv`**, **`docs/scenar/Milniky-dynamika-akt1.md`** | Tón dne 1, milníky (Vstup bod 4). |
| **`data/days.json`** | Potvrzení, že kauza běží dne 1. |
| **`docs/world-reference.md`** nebo **Story Bible** | Volitelně při doplňování reálií 1931. |
| **`.cursor/rules/cases.mdc`** | Kanon polí, volitelné `review_card` — pokud pracuješ mimo Cursor a chceš přesná pravidla. |
