# Balíček: doplnění pool případu `pool_a1_trafika` (Mrtvý trafikant)

Tento soubor slouží k **jednorázovému zadání** pro AI: obsahuje **celý prompt** ve stejném znění jako `PROMPT-doplneni-stavajicich-pool-pripadu.md` a níže **vyplněné zadání týmu** pro případ *Mrtvý trafikant*.

**K odeslání:** zkopíruj od sekce „Prompt: doplnění…“ až po konec včetně poznámky pod čarou, a **přilož** soubor `docs/Pripady/pool_a1_trafika-Mrtvy-trafikant.json` (nebo vlož jeho obsah pod zadání).

**Cíl úprav (důležité):** starší položka v `pool_cases_akt1` u této kauzy **zatím postrádá `clue_system`** a texty u rozsudků jsou spíš stručné. Dotahujeme kauzu **na stejnou úroveň** jako u vzoru *Noční směna* / *Závadný plakát* (po doplňku) — tedy **Two-Click** (`narrative_lines` u odměn, exaktní stopy dle děje), **lidské dopady u verdiktů** (B, vč. B2) a věty navazující na UI protokol, **beze změny záměru děje** a **bez** libovolného maření `effects` (číselné klíče drž tým, pokud neurčí jinak).

**Jádro děje (pro orientaci; neměň bez pokynu):** pojistný podvod (§ 197) — přihláška krátce před smrtí manžela, rozpor výpovědí o znalosti zdravotního stavu, role pojistného agenta a souseda Krejčího, pitevní vs. tvrzená „zdravost“.

**Vzor ke struktuře:** `docs/Pripady/pool_a1_tiskarna-vzor_Nocni-smena.json`, `docs/Pripady/pool_a1_vyveseni-Zavadny-plakat.json` (doplněné pátrání) a plná kauza `pool_a1_lekarna` po modernizaci.
...
## Zadání týmu (doplň / zaškrtni) — **VYPLNĚNO pro `pool_a1_trafika`**

- **ID případu (nezamění):** `pool_a1_trafika`  
- Přiložený **zdrojový JSON** (celý objekt): *`docs/Pripady/pool_a1_trafika-Mrtvy-trafikant.json` (Sp. zn. 65/1931, titulek *Mrtvý trafikant*) — vlož celý obsah souboru nebo soubor přilož.*  
- Doplň: `[x]` sekce A — **`clue_system` (nově)** + `narrative_lines` u `on_confirm` (navrhni a doplníš; stopy ve výpovědích/record s `<span class="clue" …>`, unikátní `data-clue-id` v rozsahu faktů: datum smlouvy, pitevna, matrika, provize, výpovědi) / `[x]` lidské dopady u verdictů (B, vč. B2 a inkoust dle vhodnosti u variant, které dávají smysl) / `[ ]` review_card (jen pokud tým sám přidá příznak) / `[ ]` lehká redakce výpovědí bez změny děje (volitelně) / `[x]` jiné: kontrola D — `knihovna` jen pokud přidáváš odkazy; unikátnost clue-id; **drž jména** — soused a pojistný agent = Miroslav Krejčí, manžel = Josef Bartoš; nemíchat s jinou kauzou  
- Story Bible (`InDubio_StoryBible_v2_Cursor.txt`) / world úryvek (pokud je třeba): *neposílám; při cifrách 800 Kčs, provizích, Žižkov, pojišťovna jméno Slavia drž `docs/world-reference.md` dle vhodnosti.*  
- **Kontext dne (bod 4 u Vstupu):** v `data/days.json` může být kauza u **1. dne** vedle ostatních pool případů; kauza běží dny **2–5** dle `available_days`. V `Mapa_20dni.csv` u **dne 2** je ekonomický tón a drobné pool na vedlejším slotu; *Patrani_navrh* = **std** — drž důvěryhodné úřední tón o pojistkách, chudobě, Žižkově, dětech pozůstalé, bez zbytečné velké politiky, bez měnění `effects` bez týmu.
...
| Soubor | K čemu |
|--------|--------|
| **`docs/Pripady/pool_a1_trafika-Mrtvy-trafikant.json`** | Vstupní JSON (příloha k promptu). |
| **`data/pool_cases_akt1.json`** | Cíl po schválení — nahradit stejnou položku. |
| **`data/knihovna.json`** | Při přidávání nebo kontrole `data-knihovna-id` (nové heslo jen se souhlasem týmu). |
...
| **`docs/scenar/Mapa_20dni.csv`**, **`docs/scenar/Milniky-dynamika-akt1.md`**| Tón dne 1–2, milníky (Vstup bod 4). |
| **`data/days.json`**| Skutečné zařazení v prvním týdnu. |
| **`docs/world-reference.md`** nebo **Story Bible** | Volitelně u cen a reálií. |
| **`.cursor/rules/cases.mdc`** | Kanon polí, volitelné `review_card` — mimo Cursor můžeš tento text přiložit. |
...
Doplň tento spis o **důkazní hmatatelné** stopy a tři síly pátrání, které odpovídají příběhu vdovy, zemřelého trafikanta, lékařskému záznamu, pojistné smlouvě a roli **Miroslava Krejčího**; rozsudky až na protokolární navazující věty s tím, co znamenají pro ženu, děti, pojišťovnu a důvěru v pojišťovací praxi — **nepřepisuj děj**, jen vylepš mechaniku a srozumitelnost textu.
