...
- **`pool_a1_spravce`** — OPR: doplněna **`id`** u tří výpovědí (`taussig`, `kopecka`, `holub`); rozpor `dopis_doruceni` má v `involves` **`records`** místo neexistujícího svědka `posta`; opravy skloňování **Blechy / pana Blechu** v textech Taussiga a Kopecké. Úpravy v `data/pool_cases_akt1.json` a zrcadle `docs/Pripady/pool_a1_spravce.json`.
